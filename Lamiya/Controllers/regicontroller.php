<?php
 ...
 if (empty($_POST["NID_Number"]))
      {
  		$err_NID_Number = "Please fill this form*";
		$hasError=true;
  	   }
	 ...
	 if (empty($_POST["Business_Name"]))
      {
  		$err_B_Name = "Fill this form*";
		$hasError=true;
  	}
     
  	 else
     {
  		 $B_Name=htmlspecialchars ($_POST["Business_Name"]);
  	 }
	...
	$query="insert into employee values('$FirstName',
	'$LastName','$PresentAddress','$PermanentAddress','$Gender','$Phone','$NIDNumber','$BusinessName','$Payment','$Password','$Email')";
	$result=execute($query);
	
	
	
	
	//$query="UPDATE `employee` SET `first name`='".$FirstName."',`last name`='".$LastName."',`present address`='".$PresentAddress."',`permanent address`='".$PermanentAddress."',`gender`='".$Gender."',`phone`='".$Phone."',`email`='".$Email."',`business name`='".$BusinessName."',`payment`='".$Payment."',`password`='".$Password."' WHERE`nid number`=$NIDNumber";
	//$result= execute($query);
